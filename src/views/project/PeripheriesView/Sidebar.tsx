...
const SidebarContainer = ({ isDark, className, baseUri }: { baseUri: string; isDark: boolean; className?: string }) => {
            ...
            <Sidebar.Group id={`${index}`} key={index} title={group.label}>
              ...
              >
                {group.links.map(({ to, label }, index) => (
                  <NavLink variant={isDark ? 'dark' : 'primary'} key={index} to={baseUri + to}>
                    {label}
                  </NavLink>
                ))}
              </div>
