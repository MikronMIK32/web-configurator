...
import { OptionsListWithApply } from './OptionList';
...
const selectAllOption = { value: SELECT_ALL_VALUE, content: 'Выбрать все' };
    ...
    const handleChange: Required<BaseSelectProps>['onChange'] = useCallback(
        ({ initiator, ...restArgs }) => {
            ...
            const initiatorSelected =
                ...
                (initiator.value === SELECT_ALL_VALUE && selectedDraft.length === flatOptions.length);

            if (initiator.value === SELECT_ALL_VALUE) {
                setSelectedDraft(initiatorSelected ? [] : (flatOptions as OptionShape[]));
            } else {
                setSelectedDraft(
                    initiatorSelected ? selectedDraft.filter(o => o !== initiator) : selectedDraft.concat(initiator)
                );
            }
        },
